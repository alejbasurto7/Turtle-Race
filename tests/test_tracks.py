...
# Local constant replaces the deleted N_LANES from constants.py.
# All existing tests use N=4. Wave 2 (Plan 2.1) adds parametrized N=3 cases.
N = 4
...
def test_rectangular_lane_ends_at_finish_bar():
    ...
    finish_bar = finish_line_segments(RECTANGULAR, N)[0]
...
def test_rectangular_lane_first_heading_is_north():
    for lane_idx in range(N):
        _, _, h = lane_start_pose(RECTANGULAR, lane_idx, N)
        assert h == 90
...
def test_rectangular_corner_headings_are_right_turns():
    ...
    path = build_lane_paths(RECTANGULAR, N)[0]
...
def test_spiral_first_heading_is_north():
    for lane_idx in range(N):
        _, _, h = lane_start_pose(SPIRAL, lane_idx, N)
        assert h == 90
...
def test_position_at_arc_zero_returns_start():
    for track in TRACK_NAMES:
        for lane_idx in range(N):
            path = build_lane_paths(track, N)[lane_idx]
...
def test_position_at_arc_clamps_past_end():
    path = build_lane_paths(STRAIGHT, N)[0]
    L = path_length(path)
...
def test_straight_start_line_is_perpendicular_to_heading():
    ...
    bars = start_line_segments(STRAIGHT, N)
    assert len(bars) == 1
    (x0, y0), (x1, y1) = bars[0]
    _, _, h0 = lane_start_pose(STRAIGHT, 0, N)
    vx, vy = x1 - x0, y1 - y0
    rad = math.radians(h0)
...
def test_spiral_start_returns_one_tick_per_lane():
    bars = start_line_segments(SPIRAL, N)
    assert len(bars) == N
...
def test_rectangular_finish_is_single_horizontal_bar_below_starts():
    ...
    bars = finish_line_segments(RECTANGULAR, N)
    assert len(bars) == 1
    (x0, y0), (x1, y1) = bars[0]
...
def test_straight_finish_segments_share_x_and_span_lanes():
    bars = finish_line_segments(STRAIGHT, N)
    assert len(bars) == N
...
def test_spiral_finish_is_single_bar_centered_on_origin():
    bars = finish_line_segments(SPIRAL, N)
    assert len(bars) == 1
    (x0, y0), (x1, y1) = bars[0]
...
def test_straight_start_returns_one_connecting_bar():
    ...
    assert len(bars) == 1
    (x0, y0), (x1, y1) = bars[0]
    outer_x, outer_y, _ = lane_start_pose(STRAIGHT, 0, N)
    inner_x, inner_y, _ = lane_start_pose(STRAIGHT, N - 1, N)
...
def test_rectangular_start_returns_one_tick_per_lane():
    bars = start_line_segments(RECTANGULAR, N)
    assert len(bars) == N
...
def test_boundary_stones_nonempty_for_each_track(track):
    points = boundary_stones(track, N)
    assert len(points) > 4


def test_straight_boundary_stones_form_two_parallel_rows():
    points = boundary_stones(STRAIGHT, N)
    ys = {round(y, 6) for _, y in points}
    expected = {
        round((N / 2.0) * LANE_SPACING, 6),
        round(-(N / 2.0) * LANE_SPACING, 6),
    }
    assert ys == expected
...
def test_rectangular_boundary_stones_bracket_the_racing_band():
    ...
    for x, y in boundary_stones(RECTANGULAR, N):
        outside_outer = (
            x <= bl_x - margin
            or x >= tr_x + margin
            or y <= bl_y - margin
...
def test_rectangular_returns_outer_and_inner_boundary_paths():
    paths = _boundary_paths(RECTANGULAR, N)
    assert len(paths) == 2
...
def test_spiral_boundary_first_stone_near_outer_bl_corner():
    ...
    path = _boundary_paths(SPIRAL, N)[0]
    expected_x, expected_y = path["start"]
    first_x, first_y = boundary_stones(SPIRAL, N)[0]
...
def test_boundary_stones_fit_inside_screen(track):
    half_w = WINDOW_WIDTH / 2
    half_h = WINDOW_HEIGHT / 2
    for x, y in boundary_stones(track, N):
