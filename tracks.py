...
Each track is rendered as `N_LANES` discrete paths (one per turtle) rather than
...
def _lane_coefficient(lane_idx: int) -> float:
    """Signed offset multiplier. Lane 0 = +(N-1)/2 (outermost), lane N-1 = -(N-1)/2."""
    return (N_LANES - 1) / 2.0 - lane_idx
...
def _straight_lane(lane_idx: int) -> dict:
    W, _H = _window()
    coeff = _lane_coefficient(lane_idx)
...
def _rectangular_lane(lane_idx: int) -> dict:
    ...
    """
    W, H = _window()
    o = _lane_coefficient(lane_idx) * LANE_SPACING
    ...
    closing_leg = _rectangular_finish_y() - bl_y        # BL up to the finish bar
...
def _rectangular_finish_y() -> float:
    ...
    """
    return -((N_LANES - 1) / 2.0 + 1) * LANE_SPACING
...
def _spiral_lane(lane_idx: int) -> dict:
    ...
    full_spiral = _build_spiral_legs(w, h, SPIRAL_STEP, max_pairs=_spiral_pair_cap())
    if not full_spiral:
...
def _spiral_pair_cap() -> int:
    ...
    """
    _W, H = _window()
    inner_o = -((N_LANES - 1) / 2.0) * LANE_SPACING  # most-negative offset = innermost
...
def build_lane_paths(track_name: str) -> list[dict]:
    """Return one path dict per lane, indexed 0..N_LANES-1."""
    builder = _LANE_BUILDERS[track_name]
    return [builder(i) for i in range(N_LANES)]
...
def lane_start_pose(track_name: str, lane_idx: int) -> tuple[float, float, float]:
    """Where lane `lane_idx` begins: (x, y, initial_heading)."""
    path = _LANE_BUILDERS[track_name](lane_idx)
    x, y = path["start"]
    heading = path["legs"][0][0] if path["legs"] else 0
    return x, y, heading


def start_line_segments(track_name: str) -> list[tuple[tuple[float, float], tuple[float, float]]]:
    ...
    SPIRAL: 4 separate vertical tick marks, one per lane, since each lane
    ...
    if track_name in (SPIRAL, RECTANGULAR):
        ...
        # diagonally staggered, so the 4 ticks appear at different y values
        # and read as a visible staircase rather than merging into one bar.
        ext = LANE_SPACING / 2
        bars: list[tuple[tuple[float, float], tuple[float, float]]] = []
        for lane_idx in range(N_LANES):
            x, y, _ = lane_start_pose(track_name, lane_idx)
            bars.append(((x - ext, y), (x + ext, y)))
        return bars

    x0, y0, h0 = lane_start_pose(track_name, 0)
    x1, y1, _ = lane_start_pose(track_name, N_LANES - 1)
    rad = math.radians(h0)
...
def _boundary_paths(track_name: str) -> list[dict]:
    ...
    if track_name == STRAIGHT:
        ...
        x0 = -W / 2 + TRACK_PADDING
        length = W - 2 * TRACK_PADDING
        y_top = (N_LANES / 2.0) * LANE_SPACING
        y_bot = -(N_LANES / 2.0) * LANE_SPACING
        return [
    ...
    W, H = _window()
    o = (N_LANES / 2.0) * LANE_SPACING            # half a lane outside lane 0
    bl_x = -W / 2 + TRACK_PADDING - o
    bl_y = -H / 2 + TRACK_PADDING - o
    w = (W - 2 * TRACK_PADDING) + 2 * o
    h = (H - 2 * TRACK_PADDING) + 2 * o
...
def boundary_stones(track_name: str, spacing: float = 10.0) -> list[tuple[float, float]]:
    ...
    for path in _boundary_paths(track_name):
...
def finish_line_segments(track_name: str) -> list[tuple[tuple[float, float], tuple[float, float]]]:
    """Return finish-line bar endpoints.

    STRAIGHT: a perpendicular tick at each lane's endpoint. Lanes share an
    X coordinate and have adjacent Y centers, so the 4 ticks tile into a
    ...
    paths = build_lane_paths(track_name)
    bars: list[tuple[tuple[float, float], tuple[float, float]]] = []
    ext = LANE_SPACING / 2
    for p in paths:
        total = path_length(p)
